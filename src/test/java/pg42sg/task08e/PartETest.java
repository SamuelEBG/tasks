...
public class PartETest {
    ...
    void testDpOverGrade2(){
        ...
        Course testCourse = courses.get(15);

        ArrayList<String> dpResults = diplomaProjects.stream()
                .filter(d -> d.authors.stream()
                        .filter(s -> s.courseList.containsKey(testCourse))
                        .anyMatch(s -> s.courseList.get(testCourse) >= 2))
                .map(d -> d.title)
                .collect(Collectors.toCollection(ArrayList::new));

        assertTrue(dpResults.size() > 0);
    }
    ...
    public ArrayList<Student> generateStudents(int n, ArrayList<Course> courses){
        ...
        // hashMap, together with algdat pg 4200 that is entered at the end (pg)
        for (int i = 0; i< n; i++){
